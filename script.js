(() => {
  ...
  function filteredPhotos() {
    const filtered = photos.filter(photo => activeFilter === "all" || photo.category === activeFilter);
  ...
  function renderGallery() {
    ...
    visiblePhotos.forEach((photo, index) => {
      ...
      card.setAttribute("aria-label", `${photo.title}, Großansicht öffnen`);
      ...
      fallback.innerHTML = `<div><strong>${photo.title}</strong><span>${photo.file}</span></div>`;
      ...
      meta.innerHTML = `<span>${photo.location}</span><span>${categoryName(photo.category)}</span>`;
  ...
  gallery.addEventListener("wheel", (event) => {
    if (window.innerWidth > 900 && Math.abs(event.deltaY) > Math.abs(event.deltaX)) {
      event.preventDefault();
      gallery.scrollBy({ top: event.deltaY * 1.15, behavior: "auto" });
    }
  }, { passive: false });
  ...
  function updateLightbox() {
    ...
    lightboxCounter.textContent = `${currentLightboxIndex + 1} / ${visiblePhotos.length}`;
  }
  ...
  const TAGS = {
    ...
    0x0131:"Software",0x013B:"Artist",0x8298:"Copyright",0x8769:"ExifIFDPointer",
    0x8825:"GPSInfoIFDPointer",0x9003:"DateTimeOriginal",0x9004:"CreateDate",
    0x920A:"FocalLength",0x829A:"ExposureTime",0x829D:"FNumber",0x8827:"ISO",
    0xA001:"ColorSpace",0xA002:"PixelXDimension",0xA003:"PixelYDimension",
    0xA434:"LensModel",0xA435:"LensSerialNumber",0x0001:"GPSLatitudeRef",
    ...
    0x0010:"GPSImgDirectionRef",0x0011:"GPSImgDirection",0x001D:"GPSDateStamp"
  };
  ...
  function typeSize(type){return ({1:1,2:1,3:2,4:4,5:8,7:1,9:4,10:8})[type]||1}
  ...
  function readValue(view,entryOffset,type,count,tiffStart,little){
    ...
    const valueOffset=size<=4?entryOffset+8:tiffStart+readU32(view,entryOffset+8,little);
    if(valueOffset<0||valueOffset>=view.byteLength)return null;
    if(type===2){
      const chars=[];
      for(let i=0;i<count&&valueOffset+i<view.byteLength;i++){const c=view.getUint8(valueOffset+i);if(c===0)break;chars.push(String.fromCharCode(c))}
      return chars.join("").trim();
    }
    if(type===1||type===7)return Array.from({length:count},(_,i)=>view.getUint8(valueOffset+i));
    if(type===3){const arr=[];for(let i=0;i<count;i++)arr.push(readU16(view,valueOffset+i*2,little));return count===1?arr[0]:arr}
    if(type===4){const arr=[];for(let i=0;i<count;i++)arr.push(readU32(view,valueOffset+i*4,little));return count===1?arr[0]:arr}
    if(type===5){const arr=[];for(let i=0;i<count;i++)arr.push(rational(view,valueOffset+i*8,little));return count===1?arr[0]:arr}
    if(type===9){const arr=[];for(let i=0;i<count;i++)arr.push(view.getInt32(valueOffset+i*4,little));return count===1?arr[0]:arr}
    if(type===10){const arr=[];for(let i=0;i<count;i++)arr.push(rational(view,valueOffset+i*8,little,true));return count===1?arr[0]:arr}
    return null;
  }
  function parseIFD(view,tiffStart,ifdOffset,little,limit=0){
    ...
    const count=readU16(view,absolute,little),out={};
    for(let i=0;i<count;i++){
      const entry=absolute+2+i*12;if(entry+12>view.byteLength)break;
      const tag=readU16(view,entry,little),type=readU16(view,entry+2,little),n=readU32(view,entry+4,little);
      const name=TAGS[tag]||`Tag_0x${tag.toString(16)}`;
      try{out[name]=readValue(view,entry,type,n,tiffStart,little)}catch(_){out[name]=null}
    }
    return out;
  }
  function findExif(bytes){
    ...
    while(offset+4<=bytes.length){
      ...
      if(marker===0xE1){
        ...
        if(start+6<=bytes.length&&bytes[start]===0x45&&bytes[start+1]===0x78&&bytes[start+2]===0x69&&bytes[start+3]===0x66&&bytes[start+4]===0&&bytes[start+5]===0)return bytes.slice(start+6,offset+length);
      }
      offset+=length;
    }
    return null;
  ...
  function parseExif(buffer){
    const bytes=new Uint8Array(buffer),exif=findExif(bytes);if(!exif)return {};
    const view=new DataView(exif.buffer,exif.byteOffset,exif.byteLength);if(view.byteLength<8)return {};
    const endian=String.fromCharCode(view.getUint8(0),view.getUint8(1)),little=endian==="II";
    if(!little&&endian!=="MM")return {};
    if(readU16(view,2,little)!==42)return {};
    const ifd0=parseIFD(view,0,readU32(view,4,little),little),all={...ifd0};
    if(typeof ifd0.ExifIFDPointer==="number")Object.assign(all,parseIFD(view,0,ifd0.ExifIFDPointer,little,1));
    if(typeof ifd0.GPSInfoIFDPointer==="number")Object.assign(all,parseIFD(view,0,ifd0.GPSInfoIFDPointer,little,1));
    return all;
  }
  function formatNumber(value,digits=2){return typeof value==="number"&&Number.isFinite(value)?value.toFixed(digits).replace(/\.?0+$/,""):"—"}
  function formatExposure(value){if(!Number.isFinite(value)||value<=0)return"—";if(value>=1)return`${formatNumber(value,2)} s`;return`1/${Math.round(1/value)} s`}
  function formatFocal(value){return Number.isFinite(value)?`${formatNumber(value,1)} mm`:"—"}
  function formatAperture(value){return Number.isFinite(value)?`f/${formatNumber(value,1)}`:"—"}
  ...
  async function reverseGeocode(lat,lon){
    ...
    }catch(_){return"Straßenname konnte nicht abgerufen werden"}
  }
  function buildMapUrl(lat,lon){
    const delta=.004,bbox=[lon-delta,lat-delta,lon+delta,lat+delta].join("%2C");
  ...
  async function analyzeFile(file){
    ...
    if(!/^image\/(jpeg|jpg|tiff)$/i.test(file.type)&&!/\.(jpe?g|tiff?)$/i.test(file.name)){
      alert("Bitte möglichst ein JPEG-Foto auswählen. Dieses EXIF Lab liest EXIF aus JPEG-Dateien.");return;
    }
    const objectUrl=URL.createObjectURL(file);
    preview.src=objectUrl;$("#previewFile").textContent=file.name;result.classList.remove("hidden");$("#exifStatus").textContent="Analyse…";
    try{
      const buffer=await file.arrayBuffer(),exif=parseExif(buffer);
      setText("camera",[exif.Make||"",exif.Model||""].filter(Boolean).join(" ")||"Nicht vorhanden");
      ...
      const lat=dmsToDecimal(exif.GPSLatitude,exif.GPSLatitudeRef),lon=dmsToDecimal(exif.GPSLongitude,exif.GPSLongitudeRef);
      currentGps=(lat!==null&&lon!==null)?{lat,lon,bearing}:null;
      if(currentGps){
        setText("coordinates",`${lat.toFixed(6)}, ${lon.toFixed(6)}`);
        $("#locationName").textContent="Adresse wird ermittelt…";mapButton.disabled=false;
        const address=await reverseGeocode(lat,lon);$("#locationName").textContent=address;$("#exifStatus").textContent="EXIF + GPS gefunden";
      }else{
        setText("coordinates","Keine GPS-Koordinaten in der Datei");$("#locationName").textContent="Kein GPS gefunden";
        mapButton.disabled=true;$("#exifStatus").textContent=Object.keys(exif).length?"EXIF gefunden":"Keine EXIF-Daten";
      }
    }catch(error){
      console.error(error);$("#exifStatus").textContent="Analysefehler";
  ...
  mapButton.addEventListener("click",()=>{
    ...
    const existing=document.getElementById("mapFrame");if(existing)existing.remove();
    const frame=document.createElement("iframe");
    frame.id="mapFrame";frame.title="Aufnahmeort auf OpenStreetMap";frame.loading="lazy";
    frame.style.cssText="width:100%;height:360px;border:0;border-radius:16px;margin-top:16px;background:#ddd";
    frame.src=url;$(".gps-box").insertAdjacentElement("afterend",frame);
